...
module.exports = (client, callbacks, id, data) => {
    ...
    if(data.find("iq").attrs.type == "error"){
        ...
        let errorCode = error.attrs.code;
        //400 Group join error / bad-request
        if(errorCode == '400'){
            console.log(`Client: ${client.username}: Error: ${errorCode} (bad request)`);
        }
        //406 password incorrect / password-mismatch
        else if(errorCode == '406'){ 
            onsole.log(`Client: ${client.username}: Error: ${errorCode} (incorrect password)`);
        }
        else{ console.log(`Client: ${client.username}: an error occured: ${errorCode}`); }
    }else{
        if(xmlns === "jabber:iq:register"){
            ...
            if(callback){ callback(groups, friends); callbacks.delete(id); }
            ...
            let callback = callbacks.get(id);
            if(callback){ callback(users); callbacks.delete(id); }
            ...
            if(method.startsWith("GetUsers")){
                ...
                let callback = callbacks.get(id);
                if(callback){ callback(parsedUsers); callbacks.delete(id); }
                ...
                let callback = callbacks.get(id);
                if(callback){ callback(parsedGroups); callbacks.delete(id); }
            }
        }
    }
};