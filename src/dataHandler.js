module.exports = (data, client) => {
    ...
    if(data.find("k")){
        ...
        /*{
            ...
            if(data.find("body")){
                client.emit("receivedgroupmsg", group, user, data.find("body").string)
                ...
                if(status.includes("left") || status.includes("removed")){
                    user = client.users.find((user) => {return user.jid === data.find("status").attrs.jid})

                    client.emit("userleftgroup", group, user, null)
                }else if(status.includes("joined")){
                    client.getJidInfo(data.find("status").attrs.jid)

                    //i can't return user info since i have to getJidInfo first for a user that just joined, return the
                    //JID until i find a way
